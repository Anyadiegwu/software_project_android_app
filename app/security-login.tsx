...
export default function SecurityLoginScreen() {
  ...
  const handleSignIn = async () => {
    if (!badge.trim() || !password.trim()) {
      Alert.alert('Missing Fields', 'Please enter your badge number / email and password.');
      return;
    }
    ...
    try {
      const loginUrl = `${BASE_URL}/api/auth/security/login`;
      const response = await fetch(loginUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          identifier: badge.trim(),
          password,
        }),
      });
      ...
      // Save real token + user + session for auto-launch
      await AuthStorage.saveSession(data.token, data.user, trustedDevice);

      // Keep your existing saveUserProfile call so nothing else breaks
      await saveUserProfile({
        displayName: data.user.name || data.user.badgeNumber || badge.trim(),
        email: data.user.email || badge.trim(),
