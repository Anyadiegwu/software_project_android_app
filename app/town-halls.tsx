...
export default function TownHallsScreen() {
  ...
  const fetchUpcoming = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch(`${BASE_URL}/api/townhalls/upcoming`, {
      ...
      for (const th of data) {
        try {
          const statusRes = await fetch(`${BASE_URL}/api/townhalls/${th._id}/registration-status`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (statusRes.ok) {
  ...
  const fetchPast = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch(`${BASE_URL}/api/townhalls/past`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error(`Server error ${res.status}`);
      const data: ApiTownHall[] = await res.json();
  ...
  const handleRSVP = async (townHallId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/townhalls/${townHallId}/${endpoint}`, {
        method: 'POST',
        headers: {
