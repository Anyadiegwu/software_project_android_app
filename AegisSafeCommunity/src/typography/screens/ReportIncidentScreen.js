...
export default function ReportIncidentScreen({ navigation }) {
    ...
    const submitReport = async () => {
        ...
        try {
            ...
            const createUrl = (process.env.EXPO_PUBLIC_BASE_URL || 'http://10.170.172.2:5000') + '/api/reporter/report';
            ...
            if (evidenceFiles.length > 0 && reportId) {
                try {
                    ...
                    const baseUrl = (process.env.EXPO_PUBLIC_BASE_URL || 'http://10.170.172.2:5000') + '/api/reporter/report';
                    const evidenceUrl = `${baseUrl}/${reportId}/evidence`;
