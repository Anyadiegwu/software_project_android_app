...
export default function MainHomeScreen({ navigation }) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            try {
                const analyticsUrl = (process.env.EXPO_PUBLIC_BASE_URL || 'http://10.170.172.2:5000') + '/api/reporter/analytics';
            ...
            try {
                const baseUrl = (process.env.EXPO_PUBLIC_BASE_URL || 'http://10.170.172.2:5000') + '/api/reporter/community-watch';
                const communityUrl =
