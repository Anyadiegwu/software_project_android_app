...
// // ─── Types ────────────────────────────────────────────────────────────────────
// // Raw shape returned by GET /api/reporter/community-watch
// interface ApiReport {
//   _id: string;
...
// // ─── Helpers ──────────────────────────────────────────────────────────────────
// /** Map API urgency / type → display severity */
...
//  * Returns undefined if either point is missing.
//  */
// function distanceKm(
...
// /** Normalise an ApiReport into the WatchAlert shape the UI expects */
// function normalise(
//   report: ApiReport,
//   userLat?: number,
//   userLng?: number,
...
//   // Build a readable title from category + type
...
//     const km = distanceKm(userLat, userLng, report.location?.lat, report.location?.lng);
//     if (km != null) distance = `${km.toFixed(1)} km`;
//   }

//   // Deterministic reaction counts derived from the report id (purely cosmetic)
...
// // ─── API call ─────────────────────────────────────────────────────────────────
// // async function fetchCommunityWatch(
// //   token: string,
// //   lat?: number,
// //   lng?: number,
// // ): Promise<ApiReport[]> {
// //   const params = lat != null && lng != null ? `?lat=${lat}&lng=${lng}` : '';
// //   const res = await fetch(`${API_BASE}/reporter/community-watch${params}`, {
...
//   lng?: number,
// ): Promise<ApiReport[]> {
//   let url = `${API_BASE}/reporter/community-watch`;
  
//   // Only add params if both lat and lng exist and are valid numbers
...
// /**
//  * Props expected from the parent navigator / auth context.
//  * Swap `token` for however you expose it (AsyncStorage, context, etc.).
//  */
// interface Props {
//   token: string;            // JWT from login
//   userLat?: number;         // optional — enables distance labels & radius filtering
//   userLng?: number;
// }

// export default function CommunityWatchScreen({ token, userLat, userLng }: Props) {
//   const router = useRouter();
//   const [activeTab, setActiveTab] = useState<Tab>('ALL');
//   const [search, setSearch] = useState('');
...
//   // ── Fetch on mount (and whenever coords change) ──
//   const load = useCallback(async () => {
//     try {
//       setLoading(true);
//       setError(null);
//       const raw = await fetchCommunityWatch(token, userLat, userLng);
//       setAlerts(raw.map(r => normalise(r, userLat, userLng)));
//     } catch (err: any) {
...
//       (activeTab === 'ALERTS'  && a.severity === 'HIGH') ||
//       (activeTab === 'UPDATES' && a.severity === 'UPDATE') ||
//       (activeTab === 'TOWN HALLS' && false); // no town-hall reports in this feed
//     const matchesSearch =
//       search === '' ||
//       a.title.toLowerCase().includes(search.toLowerCase()) ||
//       a.body.toLowerCase().includes(search.toLowerCase());
...
async function fetchCommunityWatch(token: string): Promise<ApiReport[]> {
  const url = `${API_BASE}/reporter/community-watch`;
